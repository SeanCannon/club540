...
function updateUser (req, res) {
  ...
  unrealisticBirthday.setYear(nowDate.getFullYear() - 120);
  
  req.checkBody('profile.firstName', 'First name must contain only letters').notEmpty().isAlpha();
  req.checkBody('profile.lastName',  'Last name must contain only letters').notEmpty().isAlpha();
  req.checkBody('profile.birthday',  'Please choose a valid birthday').isDate();
  req.checkBody('profile.birthday',  'Too old').isAfter(unrealisticBirthday);
  req.checkBody('profile.birthday',  'Sorry you are too young').isBefore(nowDate);
